...
namespace MonitAI.Core
{
    ...
    public class GeminiService : IDisposable
    {
        ...
        private async Task<GeminiAnalysisResult> AnalyzeWithSessionAsync(List<string> imagePaths, string userRules)
        {
            ...
            string prompt = $@"
あなたは監視エージェントです。以下のルールに基づいて画像を分析し、ユーザーが「現在進行系で」ルールに違反しているか判定してください。
...
1. **メインアクティビティの特定**:
   ...
   - 画面の中央、または大きく表示されているウィンドウの内容だけで判断してください。
   - 特に、**デスクトップにあるショートカットやフォルダ**、タスクバーのアイコンは、開かれていない限り無視してください。
...
{{
  ...
  ""Reason"": ""判定理由（状況分析を含めて記述、違反でない場合は'作業中'など）""
}}
...
1. **メインアクティビティの特定**:
   - 画面上の「小さなアイコン」「背景」「脇にある広告」「ブラウザのタブ」は無視してください。
   - 画面の中央、または大きく表示されている「ウィンドウ」の内容だけで判断してください。

2. **誤検知の防止**:
   - 動画サイト（YouTubeなど）のロゴやリンクが画面の隅に映っているだけでは「違反」にしないでください。
   - 勉強や業務のサイトに表示されている「広告バナー」は違反の対象外です。ユーザーがそれをクリックして視聴していない限り、無視してください。
   - 【重要】デスクトップにある「ショートカットアイコン」や、タスクバーの「起動していないアイコン」は違反とみなさないでください。フォルダ名なども原則として使用の証拠にはなりません。
   - そのアプリが「アクティブウィンドウとして開かれている」または「明らかに操作中である」場合のみ違反と判断してください。
